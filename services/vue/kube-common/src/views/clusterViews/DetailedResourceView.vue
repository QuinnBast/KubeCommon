<script setup>
import {BContainer, BCard, BCardText, BBadge, BAccordion, BAccordionItem, BTable} from "bootstrap-vue-next";
...
function getContainerStatus(containerName) {
  var containerStatus = props.item.status.containerStatuses.find((item) => item.name === containerName)
  var isReady = containerStatus.ready ? "Ready" : "Not Ready"
  return isReady + "  Restarts: " + containerStatus.restartCount
}

function getResources(container, resourceType) {
  if(container.resources.requests !== undefined && container.resources.requests[resourceType] !== undefined) {
    if(container.resources.limits !== undefined && container.resources.limits[resourceType] !== undefined) {
      return container.resources.requests[resourceType] + ' - ' + container.resources.limits[resourceType]
    }
  ...
  <BContainer>
    ...
    >
      <BCardText>
        ...
        Labels: <BBadge v-for="(value, key) in props.item.metadata.labels" :key="label">{{key}}:{{value}}</BBadge><br/>
        Annotations: <BBadge v-for="(value, key) in props.item.metadata.annotations" :key="label">{{key}}:{{value}}</BBadge><br/>
        Pod IP: {{ props.item.status.podIP }}<br/>
        Managed By: <p class="link-info" @click="showManagedResource(props.item)">{{getManagedBy(props.item)}}</p>
        <hr/>
        {{ getContainerLength() }} Containers
          <BAccordion v-for="container in props.item.spec.containers">
            <BAccordionItem :title="container.name + '          ' + getContainerStatus(container.name)" :key="container.name">
              {{ container }}
            </BAccordionItem>
          </BAccordion>
        <hr/>
        {{ getVolumeMountLength() }} Volume Mounts
        <BAccordion v-for="volumeMount in props.item.spec.volumeMounts">
          <BAccordionItem :title="volumeMount.name" :key="volumeMount.name">
            Container
            {{ volumeMount }}
          </BAccordionItem>
        </BAccordion>
        ...
        <h3>Resources</h3>
        <BContainer>
          <div v-for="container in props.item.spec.containers">
